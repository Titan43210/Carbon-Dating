...
const Navbar = () => {
    ...
    const menu = [
        { id: 1, text: 'HOME', to: '/' },
        { id: 2, text: 'DATING ADVICE', to: '/dashboard' },
    ]
    ...
    return (
        <header className={changeHeader ? "bg-white fixed z-50 top-0 left-0 w-full shadow-md transition duration-500" : "bg-transparent fixed z-50 top-0 left-0 w-full transition duration-500"}>
            {/* desktop nav  */}
            <nav className="flex bg-black items-center max-w-screen-xl mx-auto px-6 py-3">
                {/* brand  */}
                ...
                <div className="hidden md:flex lg:flex space-x-8">
                    <ul className="flex items-center space-x-4">
                        {menu.map(item => (
                            <li key={item.id}>
                                <NavLink exact to={item.to} className="text-lg text-white poppins" activeClassName="border-b-4 border-white text-white">{item.text}</NavLink>
                            </li>
                        ))}
                    </ul>
